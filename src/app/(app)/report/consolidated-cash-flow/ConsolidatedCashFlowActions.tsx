...
import type { ConsolidatedCashFlow } from "@/lib/consolidated-report";
...
export default function ConsolidatedCashFlowActions({ report, locale, baseCurrency }: { report: ConsolidatedCashFlow; locale: Locale; baseCurrency: string }) {
  ...
  function printReport() {
    ...
    const lines: [string, number, boolean][] = [
      [isZh ? "经营活动现金流" : "Operating Cash Flow", report.ocf, false],
      [isZh ? "投资活动现金流净额" : "Net Investing Cash Flow", report.icf, false],
      [isZh ? "筹资活动现金流净额" : "Net Financing Cash Flow", report.fcf, false],
    ...
    win.document.write(
      `<!DOCTYPE html><html><head><title>${escapeHtml(report.companyName)} - ${isZh ? "合并现金流量表" : "Consolidated Cash Flow Statement"}</title><style>${styles}</style></head><body>` +
        `<h1>${escapeHtml(report.companyName)}</h1>` +
        `<div class="meta">${escapeHtml(isZh ? "合并现金流量表" : "Consolidated Cash Flow Statement")} · ${isZh ? "会计年度" : "Fiscal Year"} ${report.year} · ${escapeHtml(isZh ? "生成于" : "Generated")} ${report.generatedAt.toISOString().slice(0, 16).replace("T", " ")}</div>` +
        `<h2>${escapeHtml(isZh ? "现金流量表明细" : "Cash Flow Detail")}</h2><table>${rowsHtml}</table>` +
        `</body></html>`
    );
    win.document.close();
    win.focus();
  ...
  function exportReport(format: "csv" | "xlsx") {
    const columns: ExportColumn[] = [
      { key: "label", label: isZh ? "项目" : "Item" },
      { key: "value", label: isZh ? "金额" : "Amount" },
    ];
    const rows: Record<string, unknown>[] = [
      { label: isZh ? "经营活动现金流" : "Operating Cash Flow", value: report.ocf },
      { label: isZh ? "投资活动现金流净额" : "Net Investing Cash Flow", value: report.icf },
      { label: isZh ? "筹资活动现金流净额" : "Net Financing Cash Flow", value: report.fcf },
      { label: isZh ? "现金净变动" : "Net Change in Cash", value: report.netChange },
    ];
    const filename = `consolidated-cash-flow-${report.year}`;
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute right-0 z-10 mt-1 min-w-[140px] overflow-hidden rounded-lg border py-1 shadow-lg" style={{ borderColor: "var(--border-strong)", background: "var(--surface)" }}>
            <button onClick={() => exportReport("csv")} className="block w-full px-3 py-1.5 text-left text-[12.3px]" style={{ color: "var(--ink-900)" }}>
              CSV
            </button>
            <button onClick={() => exportReport("xlsx")} className="block w-full px-3 py-1.5 text-left text-[12.3px]" style={{ color: "var(--ink-900)" }}>
              Excel (.xlsx)
            </button>
          </div>
        )}

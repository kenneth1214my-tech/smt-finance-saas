...
import type { AuditScope } from "@/lib/audit";
import type { Locale } from "@/lib/i18n/dictionaries";

const TABS: { value: AuditScope; zh: string; en: string }[] = [
  { value: "all", zh: "全部", en: "All" },
  { value: "hq", zh: "集团总部", en: "Group HQ" },
  { value: "subsidiary", zh: "子公司", en: "Subsidiaries" },
];

export default function ScopeFilter({ scope, locale }: { scope: AuditScope; locale: Locale }) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const isZh = locale === "zh" || locale === "zh-Hant";

  function onSelect(next: AuditScope) {
